...
def train(args, dataloader_train, dataloader_val, models, g_optim, d_optim, device):
    ...
    for idx in range(args.iter):
        ...
        sampled_mask  = torch.FloatTensor(np.random.randint(0,2, (img.size(0), 1)) * 1.0).to(device)
        ...
        fake_img_pred, real_img_pred, bce = model(img,sampled_ratio,label,target_mask, ada_aug_p = ada_aug_p, train_discriminator = True)
        ...
        g_loss = model(img,sampled_ratio,label,target_mask, ada_aug_p = ada_aug_p, train_generator = True)
        g_loss = g_loss.mean() 
